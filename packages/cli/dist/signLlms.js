 ...
 */
export async function runSignLlms(options) {
    const llms = await readLlms(options.cwd);
    if (!llms?.path) {
        throw new Error("Missing llms.txt. sign-llms signs an existing manifest and does not create one. Looked in llms.txt, .well-known/, public/, static/, docs/, and src/.");
    }
    const keys = await readKeyPair(options.cwd);
    if (!keys) {
        throw new Error("Missing .agentic-trust/private-key.pem. sign-llms rewrites the did.json JWS with the existing key and does not generate a new one.");
    }
    ...
    if (didFiles.length === 0) {
        throw new Error("Missing did.json. sign-llms only rewrites the JWS in an existing did.json (public/.well-known/did.json or .well-known/did.json).");
    }
    ...
    if (llms.domain && normalizeDomain(llms.domain) !== domain) {
        throw new Error(`llms.txt Domain (${llms.domain}) does not match did.json (${domain}). sign-llms did not rewrite the JWS.`);
    }
    ...
    for (const document of documents) {
        ...
        if (vmPem && normalizePem(vmPem) !== normalizePem(keys.publicKeyPem)) {
            throw new Error("did.json public key does not match .agentic-trust/public-key.pem. sign-llms does not rotate keys.");
        }
    }
    for (const document of documents) {
        const next = {
...
async function findDidFiles(cwd, publicDir) {
    const relatives = [
...
function domainFromDidId(id) {
    ...
    if (!host)
        throw new Error(`did.json id is not a usable did:web (${id}).`);
    return normalizeDomain(host);
}
function normalizePem(pem) {
    return pem.replace(/\r\n/g, "\n").trim();
}
//# sourceMappingURL=signLlms.js.map