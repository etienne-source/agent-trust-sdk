...
export function verifyPageUrl(domain) {
 ...
 */
export function renderBadge(domain) {
 ...
 */
export async function embedBadge(cwd, domain) {
    ...
    for (const file of files) {
        ...
        if (html.includes(href) || html.includes(BADGE_LABEL))
            return { status: "present" };
        if (html.includes("</footer>"))
            ranked.push({ file, html, slot: "footer" });
        else if (html.includes("</body>"))
            ranked.push({ file, html, slot: "body" });
    }
    ranked.sort((left, right) => {
        if (left.slot !== right.slot)
            return left.slot === "footer" ? -1 : 1;
...
async function findLayoutFiles(cwd) {
    const found = [];
    async function walk(dir, depth) {
        if (depth > 6 || found.length > 40)
            return;
        let entries;
        try {
