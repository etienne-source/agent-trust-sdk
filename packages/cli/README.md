...
> **Do not install `trustflow-sdk`.** That unscoped package is an unrelated logging package. The CLI command is `npx trustflow init` from `@trustflow/cli`. Install from GitHub until the tag publish succeeds: `github:etienne-source/agent-trust-sdk`.

## Install

`@trustflow/cli` depends on `@trustflow/sdk` with `workspace:*`. Clone `github:etienne-source/agent-trust-sdk` so that link resolves. npm does not install a workspace package from a git URL, and `pnpm add github:etienne-source/agent-trust-sdk#path:/packages/cli` cannot resolve `workspace:*` on its own.
...
`dist/` is committed, so the binary runs after `pnpm install` links the SDK. Use `pnpm --filter @trustflow/cli exec trustflow --help` from the clone.
...
4. Generates an Ed25519 `did:web` key with `@trustflow/sdk` (`createSignedDidDocument`), writes `<public>/.well-known/did.json` (and the root mirror), and stores `private-key.pem` in `.agentic-trust/` with mode `0600`. That directory is appended to `.gitignore`.
