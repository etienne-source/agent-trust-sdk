...
export type NotifyVerifiedReason =
  | "sent"
  | "not_complete"
  | "webhook_unset"
  | "webhook_rejected"
...
export function isCompleteVerification(notice: VerifiedDomainNotice): boolean {
  const max = notice.maxScore ?? VERIFIED_SCORE_COMPLETE;
  return (
    String(notice.status).toUpperCase() === "VERIFIED" &&
    notice.score === VERIFIED_SCORE_COMPLETE &&
    max === VERIFIED_SCORE_COMPLETE
  );
}

export function buildVerifiedNotifyPayload(notice: VerifiedDomainNotice, now = new Date()): VerifiedNotifyPayload {
...
function webhookHostBlocked(hostname: string): boolean {
 ...
 */
export async function notifyVerifiedDomain(
  notice: VerifiedDomainNotice,
  options: NotifyVerifiedDomainOptions = {}
): Promise<NotifyVerifiedDomainResult> {
  ...
  const configured =
    options.webhookUrl !== undefined ? options.webhookUrl : process.env[VERIFIED_NOTIFY_ENV];
  const webhook = (configured ?? "").trim();
  if (!webhook) {
    logger.info(
      `[AgenticTrust] ${domain} is 100/100 VERIFIED. ${VERIFIED_NOTIFY_ENV} is unset, so no webhook was sent. Sharing on X is a separate explicit step and is not performed by this hook.`
    );
  ...
  if (
    !assertHttpsEndpoint(webhook) ||
    url.username ||
    url.password ||
    webhookHostBlocked(url.hostname)
  ) {
    logger.warn(
      `[AgenticTrust] Refusing webhook host ${url.hostname}. Notifications are HTTPS webhooks you configure. This hook does not post to X.`
    );
    return { sent: false, reason: "webhook_rejected" };
  }
