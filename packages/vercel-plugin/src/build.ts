...
export function agenticTrustBuildCommand(appBuild = "next build"): string {
 ...
 */
export function withAgenticTrustVercelConfig<T extends AgenticTrustVercelConfig>(
  config: T,
  appBuild = "next build"
...
function resolveDomain(options: AgenticTrustVercelBuildOptions, env: NodeJS.ProcessEnv): string {
  const raw =
    options.domain?.trim() ||
...
function assertPrivateKey(pem: string): "Ed25519" | "ES256" {
  let key;
  try {
    key = createPrivateKey(pem);
  } catch {
    throw new Error(
      "AGENTIC_TRUST_PRIVATE_KEY could not be read. Expected an unencrypted Ed25519 or P-256 PKCS#8 PEM. It is not written."
    );
  }
...
async function defaultOutDir(cwd: string): Promise<string> {
  if (await pathExists(path.join(cwd, "public"), "dir")) return "public";
  for (const name of NEXT_CONFIGS) {
    if (await pathExists(path.join(cwd, name), "file")) return "public";
  }
  return ".";
}
 ...
 */
export async function runAgenticTrustVercelBuild(
  options: AgenticTrustVercelBuildOptions = {}
): Promise<AgenticTrustVercelBuildResult> {
  ...
  if (!privateKeyPem) {
    throw new Error(
      "AGENTIC_TRUST_PRIVATE_KEY is not set. Add the Ed25519 or P-256 PEM as a Vercel sensitive environment variable. Do not commit it and do not pass it on a command line."
    );
  }
