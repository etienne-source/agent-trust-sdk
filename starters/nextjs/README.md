# AgenticTrust Next.js starter

Minimal [Next.js](https://nextjs.org) App Router app with a placeholder `did:web` identity, `llms.txt`, Cursor rules, and [`@agentic-trust/next-plugin`](../../packages/next-plugin/README.md).
...
> Do not install `trustflow-sdk`. That npm package is unrelated. `@agentic-trust/next-plugin` is not on npm yet. This starter depends on `github:etienne-source/agent-trust-sdk#path:/packages/next-plugin`.
...
`next.config.ts` wraps the Next config with `withAgenticTrust` and sets `turbopack: {}`. Next.js 16 uses Turbopack for `pnpm dev` and exits when a plugin adds a `webpack` hook without that key. The identity check still runs when the config loads. While `NODE_ENV` is `development`, the plugin checks:
...
Then generate a real signature. The command you want, once `@agentic-trust/cli` is on npm:
...
Until that scope is published, run the same `init` from a clone. The CLI depends on `@agentic-trust/sdk` with `workspace:*`, so install the repository rather than only the CLI folder:
...
Inside this monorepo, from `starters/nextjs`:
...
```text
next.config.ts                 withAgenticTrust
