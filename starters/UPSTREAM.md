...
In the template README, name **Trustflow** as the protocol and **Trustflow Systems** as the hosted registry, and point identity setup at `npx @trustflow/cli@latest init`. After `init`, copy `.well-known/did.json` to `public/.well-known/did.json`, because Next.js serves `public/`.
...
- Placeholders still say `REPLACE_ME`, or the maintainer replaced them with `npx @trustflow/cli@latest init` and copied the signed DID into `public/`.
