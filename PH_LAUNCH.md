...
Draft for the Trustflow open-source launch. Trustflow Systems is the hosted registry named in the same post. No metrics are claimed because none are published with this draft.
...
AI agents quote prices and call tools from domains they have never checked. Trustflow is the open protocol, SDK, CLI, and middleware for a W3C `did:web` document and a compact JWS on that document. Signatures are Ed25519 (`EdDSA`) or P-256 (`ES256`). `alg: none` and HMAC (`HS*`) proofs are rejected.

Trustflow Systems hosts the registry at trustflow.systems and api.trustflow.systems. The open code lives in the Trustflow monorepo. The registry is the hosted service. The badge reads: Verified Domain Context | Trustflow.
...
Trustflow is the open protocol. Trustflow Systems is the registry we run for it.
...
Registration talks to `https://api.trustflow.systems` (`POST /v1/register`, then `POST /v1/register/confirm`). The public page for a domain is `https://trustflow.systems/verify/<domain>`. The badge text is `Verified Domain Context | Trustflow`.
